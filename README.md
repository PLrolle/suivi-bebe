...
Créer un nouveau document et lui donner un nom (ex : `suivi-bébé`). Laisser le document vide — l'app crée automatiquement les tables au premier lancement.
...
Cliquer sur l'**avatar en haut à droite** → **Paramètres du compte** → section **Clés API** → **Ajouter une clé** → copier la clé (sans le préfixe `Bearer`).

> 💡 L'**ID du document** et la **clé API** sont tous deux accessibles depuis l'avatar en haut à droite de l'interface Grist.
...
Au premier lancement, l'app affiche directement l'écran de configuration. Renseigner :
...
> ⚠️ **Chaque parent configure l'app sur son téléphone** avec les mêmes identifiants Grist et la même URL proxy.
